...
- SBR、求解日志、提取表和短期图表放在 `tmp/irw_passive_multispeed_simpack_slv_comparison/`；大型
  运行结果同样不进入 Git。相关数值工件未被 Git 索引，具体记录需要重新仿真获得。
...
- SIMPACK SBR 为 binary64、`3001` 个 10 ms 样本；八路电机转矩逐点为零。ORVD 为 `60001` 个
  ...
  `0.0151–0.0226 mrad`。
- 本轮数值工件未被 Git 索引，具体记录需要重新仿真获得；结果已经人工签收并写回仓库根 README。
...
- 该直线工况的 SIMPACK 积分为 `52.58 s / 0.571×`，完整进程为 `60.07 s / 0.499×`；ORVD 严格
  ...
  对比口径，不构成性能配方推荐。
- 该直线工况的数值工件未被 Git 索引，具体记录需要重新仿真获得；其中 t9—t10 收紧诊断表明
  起振时刻与原生相位未收敛，而限界振荡包络和碰撞冲量分布较稳定。
...
- 该 R600 工况的数值工件未被 Git 索引，具体记录需要重新仿真获得。
...
- 该直线工况的 SIMPACK 积分为 `14.39 s / 0.556×`，完整进程为 `18.20 s / 0.439×`；ORVD 推进为
  `36.64 s / 0.218×`，完整进程为 `38.47 s / 0.208×`。本轮数值工件未被 Git 索引，具体记录
  需要重新仿真获得。
...
- ORVD 推进为 `47.13 s / 0.637×`，完整进程为 `55.68 s / 0.539×`；SIMPACK SODASRT2
  ...
  实时系数。本轮数值工件与逐算法两线图未被 Git 索引，具体记录与图件需要重新仿真获得。各路径
  使用的线程数和输出密度不同，这些数字是现行运行的实测墙钟，不是等核、等输出负担的算法基准。
...
- R800 工况的数值工件未被 Git 索引，具体记录需要重新仿真获得。
...
- 直线/AAR6/160 km/h 工况的数值工件未被 Git 索引，具体记录需要重新仿真获得。
...
- 直线/ERRI low/200 km/h 工况的数值工件未被 Git 索引，具体记录需要重新仿真获得。
