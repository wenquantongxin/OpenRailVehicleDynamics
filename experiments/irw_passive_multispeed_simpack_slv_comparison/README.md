...
30 s 数值路径已经完成，跨软件比较严格止于 `8.71 s` 的共同可比线路范围；该专项结果即为
R1000 工况的现有证据，不再通过稳定编排入口重跑。R800／100 km/h 已按稳定入口完成 30 s
对拍和完整响应图。本文不代表多速度工况已经整体通过资格。
...
不能看完结果再选择。高频运行是否只改变输出观察而不改变端点和积分统计，也要用同场小窗复核。

R1000／120 km/h 专项已表明，默认 SODASRT2 在关闭 `Meet output points` 时约 `1.13 s` 终止，
打开后可完成 30 s，且宏观响应与定步长及 DOPRI5 路径接近。因此从 R800／100 km/h 工况起，
后续场景的临时 SPCK 统一显式设置 `slv.integ.meetop=1`，作为更高速度
工况的 SIMPACK direct-SLV 稳定性执行口径。该设置不把 100 Hz 输出钟变成固定积分步长，但会使
求解器在通信点计算运动方程，从而改变内部步进和计时口径。编排入口已经落实该字段，并将精确临时
模型差异写入对应工件；既有 60、80 和 120 km/h 结果未为统一此设置而重跑。
...
身份，不再仅按不平顺标识选择积分配方。R300/AAR5/60 km/h、直线或 R600/AAR5/80 km/h、
R800/AAR5/100 km/h、直线或 R1000/AAR6/120 km/h、直线/AAR6/160 km/h 以及
直线/ERRI low/200 km/h 分别绑定线路几何、冻结不平顺、启动速度和逐实例私有积分配方；不匹配的线路、
...
仿真时长、输出时钟和共同比较边界由本实验的场景目录绑定并写入运行清单，不属于 ORVD 私有配方。

不开放任意字符串拼装，不把数值容差变成公共产品配置，也不把 R300 已验证的 BDF-5/t8 结果外推为
所有速度和曲线半径的默认结论。直线／80 km/h、R600／80 km/h、R800／100 km/h、直线与
R1000／120 km/h、直线／160 km/h 以及直线／ERRI low／200 km/h
分别持有独立的五阶严格对比配方，首轮数值均为
...
- R1000 的四路专项已超过原计划 8 s 短窗所需的科学证据；保留其一次性研究属性，不再为形成稳定
  编排形式而重复运行。
...
- R800/AAR5/100 km/h 已使用 SODASRT2＋`Meet output points` 和对应的 ORVD 五阶严格配方
  完成被动 30 s 对拍。两端四轴初始站位完全相同，八轮前向角速度最大差为
  `1.42e-14 rad/s`，八路驱动转矩逐点为零。末窗最大轴桥站位为 `836.736 m`，未越过
  AAR5 的 `1100 m` 共同定义域。
- 原生同时横移 RMS 差为 `0.0263–0.0393 mm`，最大绝对差不超过 `0.2521 mm`；摇头 RMS 差为
  `0.0487–0.1316 mrad`，最大绝对差不超过 `0.5440 mrad`，全窗未施加时移。八轮逐轮接触斑汇总
  三向力 RMS 差为 `N=0.958 kN`、`Tx=0.237 kN`、`Ty=0.414 kN`；力仍只作 100 Hz 诊断。
- 两端只在 `FF_R, t=1.73 s` 观察到同一短时零接触事件：SIMPACK 的 100 Hz 输出钟上为 1 点，
  ORVD 的 0.5 ms 输出钟上为连续 2 点。这支持事件身份一致，不用不同采样钟直接比较持续时间。
- SIMPACK 在 `Meet output points` 开启后完成 `205043` 个积分步，校正器迭代错误为零；本轮墙钟受
  并行外部负载影响，工件保留原始计时，本文不从它们推导实时性结论。
- R800 工况工件位于外置实验数据根
  `ORVD_Assist/irw-passive-multispeed-simpack-slv-comparison-20260821/stage3-r800-frozen-aar5-100kmph-30s/`。
- 直线/AAR6/160 km/h 已使用 SODASRT2＋`Meet output points` 和对应的 ORVD 五阶严格配方完成
  被动 6 s 对拍。两端四轴初始站位完全相同，八轮前向角速度最大差为 `1.42e-14 rad/s`，八路驱动
  转矩逐点为零；末窗最大轴桥站位为 `276.616 m`，未越过 AAR6 的 `300 m` 共同定义域。
- 两端自身响应包络接近：四轴横移自身 RMS 分别为 SIMPACK `5.802–8.228 mm`、ORVD
  `5.573–8.154 mm`；摇头自身 RMS 分别为 `1.391–6.777 mrad`、`1.347–6.603 mrad`。
  但进入轮缘—钢轨限界碰撞后原生相位分离，同时间横移 RMS 差为 `3.674–8.323 mm`，摇头 RMS 差为
  `1.563–5.982 mrad`。前两轴 ORVD 达到 `1 mm` 横移约早 `0.24–0.25 s`，后两轴只早
  `0.03 s`；未施加时移，也不以包络接近替代原生同时间资格。
- 两端八轮均出现间歇零接触。SIMPACK 的 100 Hz 输出钟观测到各轮最长连续 `10–40 ms`，ORVD 的
  0.5 ms 输出钟为 `8–20.5 ms`；三向力中的接触切换尖峰只作诊断，不用不同采样钟直接比较峰值或
  失联持续时间。该工况说明提高速度后，两端仍给出相近的限界振荡强度，但初始扰动和碰撞时序差异
  足以造成显著逐点差，尚未形成跨求解器原生时序闭合。
- 直线/AAR6/160 km/h 工件位于外置实验数据根
  `ORVD_Assist/irw-passive-multispeed-simpack-slv-comparison-20260821/stage4-straight-frozen-aar6-160kmph-6s/`。
- 直线/ERRI low/200 km/h 已使用 SODASRT2＋`Meet output points` 和对应的 ORVD 五阶严格配方完成
  被动 8 s 对拍。两端四轴初始站位完全相同，八轮前向角速度最大差为 `5.68e-14 rad/s`，八路驱动
  转矩逐点为零；末窗最大轴桥站位为 `454.330 m`，未越过 ERRI low 的 `500 m` 共同定义域。
- ERRI low 在 `50–100 m` 淡入、`100–450 m` 保持全幅、`450–500 m` 淡出；末窗只有最前轴刚进入
  淡出段。两端自身响应包络仍接近：四轴横移自身 RMS 分别为 SIMPACK `7.004–9.170 mm`、ORVD
  `7.013–8.893 mm`；摇头自身 RMS 分别为 `1.289–7.362 mrad`、`1.291–7.224 mrad`。
- 进入轮缘—钢轨限界碰撞后原生相位分离，同时间横移 RMS 差为 `6.114–10.494 mm`，摇头 RMS 差为
  `1.681–7.608 mrad`。ORVD 四轴达到 `1 mm` 横移约早 `0.12–0.20 s`；未施加时移，也不把包络
  接近写成原生时序闭合。SIMPACK 的 100 Hz 输出钟观测到各轮最长连续零接触 `10–50 ms`，ORVD
  的 0.5 ms 输出钟为 `10–21 ms`；力峰继续只作诊断。
- 直线/ERRI low/200 km/h 工件位于外置实验数据根
  `ORVD_Assist/irw-passive-multispeed-simpack-slv-comparison-20260821/stage5-straight-frozen-erri-low-200kmph-8s/`。

本示例已经完成预定的逐场运行。R600、R800 等曲线工况可以原生闭合，直线高速度 IRW 限界碰撞工况则
表现为包络接近但相位敏感；因此不作笼统的“多速度跨求解器一致”声明，也不宣称新的实时性能。
