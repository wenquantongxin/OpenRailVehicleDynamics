...
一次完整 100 s 运行发布了 `200001` 个 0.5 ms 机械观测、`10001` 个控制事件和 `1666471` 个接触斑记录。初始化、`9999` 个周期事件与终端事件严格落在整数控制时钟；四轴投影站位始终单调前进，单个机械输出间隔内的站位增量为 `8.32–13.94 mm`，未发生局部分支投影失败。
