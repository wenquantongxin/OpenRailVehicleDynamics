...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::fprintf(
            stderr,
            "usage: orvd_irw_r800_aar5_v100_better2_full_state_guidance "
    ...
    try {
        const auto summary =
            orvd::experiments::irw_crossline_full_state_guidance::
                RunIrwR800Aar5V100Better2FullStateGuidance(argv[1], argv[2]);
        std::printf(
        ...
        std::fprintf(stderr, "IRW R800 better2 experiment failed: %s\n",
                     error.what());
        return 1;
    }
}
