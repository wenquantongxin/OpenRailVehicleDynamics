...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::fprintf(
            stderr,
            "usage: "
            "orvd_irw_r300_aar5_v60_scbp_recorded_wear_champion_"
    ...
    try {
        ...
        const IrwSingleCurveFullStateGuidanceSchedule schedule({
            ...
            .profile = kScbpR300Profile,
        });
        const auto summary = RunIrwSingleCurveGuidanceSimpackRealtime(
            {
                .experiment_name =
                    "IRW R300 V60 SCBP recorded wear champion full-state "
        ...
        std::fprintf(stderr, "SIMPACK R300 SCBP experiment failed: %s\n",
                     error.what());
        return 1;
    }
}
