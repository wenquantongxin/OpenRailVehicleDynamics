...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::fprintf(
            stderr,
            "usage: orvd_irw_r300_aar5_v60_normal_differential_wheel_speed "
    ...
    try {
        const auto summary =
            orvd::experiments::irw_crossline_full_state_guidance::
                RunIrwR300Aar5V60NormalDifferentialWheelSpeed(argv[1],
                                                               argv[2]);
        std::printf(
        ...
        std::fprintf(stderr, "IRW R300 normal experiment failed: %s\n",
                     error.what());
        return 1;
    }
}
