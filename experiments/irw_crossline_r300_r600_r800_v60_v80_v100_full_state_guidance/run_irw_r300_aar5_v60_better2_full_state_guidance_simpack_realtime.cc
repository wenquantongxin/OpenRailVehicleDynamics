...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::fprintf(
            stderr,
            "usage: orvd_irw_r300_aar5_v60_better2_simpack_realtime "
    ...
    try {
        ...
        const IrwSingleCurveFullStateGuidanceSchedule schedule({
            ...
            .profile = kBetter2R300Profile,
        });
        const auto summary = RunIrwSingleCurveGuidanceSimpackRealtime(
            {
                .experiment_name =
                    "IRW R300 V60 better2 full-state guidance SIMPACK Realtime",
        ...
        std::fprintf(stderr, "SIMPACK R300 better2 experiment failed: %s\n",
                     error.what());
        return 1;
    }
}
