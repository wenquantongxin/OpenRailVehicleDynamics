...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::fprintf(
            stderr,
            "usage: orvd_irw_r600_aar5_v80_normal_simpack_realtime "
    ...
    try {
        ...
        const auto summary = RunIrwSingleCurveGuidanceSimpackRealtime(
            {
                .experiment_name =
                    "IRW R600 V80 normal differential-wheel-speed SIMPACK Realtime",
        ...
        std::fprintf(stderr, "SIMPACK R600 normal experiment failed: %s\n",
                     error.what());
        return 1;
    }
}
