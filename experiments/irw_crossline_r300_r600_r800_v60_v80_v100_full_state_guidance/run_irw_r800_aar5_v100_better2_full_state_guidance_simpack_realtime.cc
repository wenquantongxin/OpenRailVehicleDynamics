...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::fprintf(
            stderr,
            "usage: orvd_irw_r800_aar5_v100_better2_simpack_realtime "
    ...
    try {
        ...
        const IrwSingleCurveFullStateGuidanceSchedule schedule({
            ...
            .profile = kBetter2R800Profile,
        });
        const auto summary = RunIrwSingleCurveGuidanceSimpackRealtime(
            {
                .experiment_name =
                    "IRW R800 V100 better2 full-state guidance SIMPACK Realtime",
        ...
        std::fprintf(stderr, "SIMPACK R800 better2 experiment failed: %s\n",
                     error.what());
        return 1;
    }
}
