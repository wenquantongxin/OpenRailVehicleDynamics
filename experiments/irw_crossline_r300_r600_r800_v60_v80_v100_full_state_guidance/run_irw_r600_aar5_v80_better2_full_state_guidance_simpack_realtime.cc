...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::fprintf(
            stderr,
            "usage: orvd_irw_r600_aar5_v80_better2_simpack_realtime "
    ...
    try {
        ...
        const IrwSingleCurveFullStateGuidanceSchedule schedule({
            ...
            .profile = kBetter2R600Profile,
        });
        const auto summary = RunIrwSingleCurveGuidanceSimpackRealtime(
            {
                .experiment_name =
                    "IRW R600 V80 better2 full-state guidance SIMPACK Realtime",
        ...
        std::fprintf(stderr, "SIMPACK R600 better2 experiment failed: %s\n",
                     error.what());
        return 1;
    }
}
