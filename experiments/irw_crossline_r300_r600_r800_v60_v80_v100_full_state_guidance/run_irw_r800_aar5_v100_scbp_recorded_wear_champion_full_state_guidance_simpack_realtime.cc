...
int main(int argc, char** argv) {
    if (argc != 4) {
        std::fprintf(
            stderr,
            "usage: orvd_irw_r800_aar5_v100_scbp_recorded_wear_champion_"
    ...
    try {
        ...
        const IrwSingleCurveFullStateGuidanceSchedule schedule({
            ...
            .profile = kScbpR800Profile,
        });
        const auto summary = RunIrwSingleCurveGuidanceSimpackRealtime(
            {
                .experiment_name =
                    "IRW R800 V100 SCBP recorded wear champion full-state "
        ...
        std::fprintf(stderr, "SIMPACK R800 SCBP experiment failed: %s\n",
                     error.what());
        return 1;
    }
}
