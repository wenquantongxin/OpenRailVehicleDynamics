...
function(_orvd_imported_configuration_names target output_variable)
    ...
    get_property(imported_configuration_names
        TARGET ${target} PROPERTY IMPORTED_CONFIGURATIONS)
    list(LENGTH imported_configuration_names imported_configuration_count)
    if(imported_configuration_count GREATER 0)
    ...
    foreach(configuration_name IN LISTS configuration_names)
        ...
        get_property(mapped_configuration_names
            TARGET ${target}
            PROPERTY "MAP_IMPORTED_CONFIG_${upper_configuration_name}")
        list(LENGTH mapped_configuration_names mapped_configuration_count)
        if(mapped_configuration_count GREATER 0)
...
function(_orvd_imported_artefact_is_drake target output_variable)
    ...
    foreach(artefact_property IN LISTS artefact_properties)
        # `get_property`, unlike `get_target_property`, returns an empty value
        # rather than a false-like `<variable>-NOTFOUND` sentinel for an unset
        # property. Count the resulting list: an artefact whose real file name
        # ends in `-NOTFOUND` still exists as text and still needs inspecting.
        get_property(artefact TARGET ${target} PROPERTY ${artefact_property})
        list(LENGTH artefact artefact_count)
        if(artefact_count GREATER 0)
...
function(_orvd_check_link_options product_target target is_first_party)
    ...
    foreach(option_property IN LISTS option_properties)
        get_property(options TARGET ${target} PROPERTY ${option_property})
...
function(_orvd_walk_link_closure product_target item is_first_party)
    ...
    foreach(edge_property IN LISTS edge_properties)
        get_property(edge_values TARGET ${item} PROPERTY ${edge_property})
        list(LENGTH edge_values edge_value_count)
        if(edge_value_count GREATER 0)
...
function(_orvd_collect_product_directory product_module_directory)
    ...
    list(LENGTH directory_targets directory_target_count)
    if(directory_target_count GREATER 0)
        set_property(GLOBAL APPEND PROPERTY
...
function(orvd_verify_product_targets_have_no_drake_dependency)
    ...
    list(LENGTH CMAKE_CONFIGURATION_TYPES configuration_type_count)
    list(LENGTH CMAKE_BUILD_TYPE build_type_count)
    if(configuration_type_count GREATER 0)
        set(configuration_names ${CMAKE_CONFIGURATION_TYPES})
    elseif(build_type_count GREATER 0)
    ...
    list(LENGTH product_targets product_target_count)
    if(product_target_count GREATER 0)
    ...
    if(product_target_count EQUAL 0 AND violation_count EQUAL 0)
        message(STATUS
