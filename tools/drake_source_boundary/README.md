...
链接只覆盖**当次编译成功的子集**，工具会明说这一点。G28 已通过 CMake OBJECT owners
编译整个 landed 边界，并由常驻全对象测试强制全部对象进入最终链接。
...
落盘通过数、符号表或允许列表：今天记下的数字，明天会变成一道因错误理由而通过的门。它
输出的是**源码层**运行时依赖表面，不宣称完整 ABI，也不宣称最小实现契约。
