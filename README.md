...
的安装态真实系统；H3 已解析启动已装入 `q81/v74/z2`，S1002/UIC60 型面和 IRW 接触人格已绑定，
四个轴桥的线路投影和八个独立车轮的真实轮心运动、轮轨扳手已接入完整 157 维瞬时动力学方程；
无轨道不平顺 R300 的 A 层和冻结 AAR5 的 B 层均已完成相对 SIMPACK 的 30 s 被动宏观响应资格，
并与近期 WRL 并列核对。IRW 被动迁移阶段已经闭环，1 kHz 控制系统仍未接入。
实施顺序与完成条件见
...
H3 已解析状态已装入 `q81/v74/z2`：四个轴桥仍是具有完整自由体转动状态的车轴刚体，
左右车轮各自通过具名转动副保存相对车轴的转角与转速。IRW 不使用 GZ18 的公共轮速生成关系，也不
重复保存车轮绝对角速度；后者由车轴运动与轮—轴相对转动共同决定。IRW 已用随包 S1002 JSON、
共享 UIC60 轨型、`0.43 m` 标称半径和 `0.1 mm` 源横坐标重离散形成闭合接触人格，并保留其
启用侧滚横垂输运的车型差异。四个轴桥现承担线路站位投影，八个独立车轮分别提供真实轮心运动并
承受轮轨扳手；H3/R300 下完整 157 维瞬时动力学方程已接通，并已完成无轨道不平顺的首次 10 ms
连续推进及端点闭合。A 层又完成了一次连续 30 s R300 被动运行：在各轴桥自己的
`100–450 m / 0.01 m` 站位格上，ORVD—SIMPACK 横移／摇头最大误差约为
`3.05 µm / 1.16 µrad`。B 层在相同车辆、启动、线路、积分器和采样时钟上只增加冻结 AAR5；
同站位比较的四轴横移均方根／最大误差最差为 `6.83/44.85 µm`，摇头为
`21.35/78.64 µrad`。逐点扣除 A 层基差后的不平顺增量误差最差为
`6.47/46.71 µm` 与 `21.36/78.48 µrad`。同时间峰值仍达到
`180.0 µm / 125.2 µrad`，与冻结 WRL—SIMPACK 已有站位传播差异同量级，因此不能把
同站位资格扩大为同时间高精度闭合。IRW 被动迁移至此闭环，控制系统仍属后续工作。

IRW 被动主线沿用 C++＋CVODE 的逐次 RHS 接触求值，不迁移旧接触采样保持。A 层为无轨道不平顺的 R300
`30 s`，B 层为同一冻结 AAR5 `30 s`；两层都按每轴 `100–450 m` 的 `0.01 m` 公共站位格裁决。
WRL 在约 `3.659 s` 观测到连续 13 个 `100 µs` 样本仅 7 轮接触，ORVD 本次为相邻的 14 个样本；
两者只作拓扑观察，不预设成逐样本金标。该边界不否决限定范围的宏观响应资格，也禁止声称全时程
八轮接触或安全资格。B 层轮轨力仍只作诊断，尚未达到与 SIMPACK 的高精度闭合。
