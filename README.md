...
不能称为高精度闭合；IRW 整车与控制系统尚未迁入，其建模所需的 Ball-RPY 球铰，以及采用
半角中间坐标系和共同中点加载的六分量线性衬套已经进入公共多体与力元层。实施顺序与完成条件见
...
IRW 是阶段一的第二个真实车型，而不是 GZ18 的参数变体。模型中立门面已经具备与冻结 WRL 一致的
Ball-RPY 纵向拉杆球铰，以及采用半角中间坐标系和共同中点加载的完整六分量衬套；规划中的车型还包含
八个独立车轮自旋和两条 Maxwell 状态。这些能力先以模型中立公共接口落地，再由
