...
当前资格范围已覆盖 GZ18 在直线、水平、零超高条件下的无轨道不平顺 10 ms 纵切，以及冻结 AAR6
输入的 10 s 激活窗和 20 s 完整包络；四个轮对的横移与摇头均已和 SIMPACK、历史 WRL 作时序与
里程序列核对。曲线工况、IRW 与控制系统尚未完成资格验证。实施顺序与完成条件见
...
`20 s` 完整包络的精度—性能基线。下一阶段先核定 R300 线路几何、站位观察语义及超高基准，
再在同一条 R300+AAR5 `16 s` 工件上分别裁决纵向传播和车辆响应。历史报告中的
`4.9 mm RMS / 16 mm max` 来自 P040 SIMPACK 轨道参考站位与 P055 后续复算采用的轮轨有效站位
观察量；现有证据尚未证明两者同义，因此它不是轮对刚体原点误差或线路 JSON 金标。G62 将先完成
同义观察量对齐，再评价真实纵向传播。

### 已完成的 GZ18 工况

当前已完成三层真实计算：直线、水平、零超高且无轨道不平顺的 `10 ms` 首次闭环；同一车辆加入
冻结 AAR6 横／垂不平顺后的 `10 s` 激活窗；以及从 `t=0` 独立连续运行的 `20 s` 完整包络。长窗
采用 60 km/h 初始速度、`0.5 ms` 观察周期和 40001 个样本，覆盖 `50–100 m` 淡入、`100–250 m`
全幅、`250–300 m` 淡出及其后恢复。ORVD、SIMPACK 与历史 WRL `b4466a40` 均按原始时序和各自
里程直接比较，不作移时、滤波、拟合、缩放或去均值。
