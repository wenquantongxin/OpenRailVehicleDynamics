...
60 km/h 已解析移动启动，以及独立串行轮轨接触核。轮轨型面基础设施包含 JSON 型面模式、与 SIMPACK
...
这还不是完整车辆仿真闭环：轮轨力尚未进入车辆每次 RHS，轨道不平顺、长窗、IRW 与控制器也尚未
迁入。具体实施进度、当前 Goal 和完成门只在
...
| IRW 1 kHz 控制与交付 | G73–G81 | 八路轮—轴桥纯力偶、简单 PID、同人格长窗、公开场景／输出和安装应用 |
...
JSON 型面和类型化接触人格。接触核目前仍是独立产品能力，尚未进入车辆每次 RHS；具体签收状态见路书。

G55 只建立无不平顺直线 `10 ms` 的首次闭环。后续 G56–G63 将把冻结横／垂不平顺作为独立站位场
...
控制主线是同进程 1 kHz 简单 PID 与八路轮—轴桥等大反向纯力偶。历史 P179 的 100 Hz 全状态控制
加普通 `bridge_proxy` 只是另一种研究人格，不能替代尚需生成的 1 kHz 同人格 SIMPACK Realtime
