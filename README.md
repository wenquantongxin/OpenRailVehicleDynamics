...
接入真实车辆并完成 `0.5 s` P179 同人格短窗，`30 s` 控制长窗尚未开展。
实施顺序与完成条件见
...
│   ├── gz18/              GZ18 完整已资格资产
│   └── irw/               IRW 被动机械记录、H3 启动与 S1002 轮型
...
| IRW 100 Hz 控制与交付 | G73–G82 | 构架—独立车轮纯力偶、全状态轮速导向、转矩指令调理、P179 同人格长窗及产品闭包 |

完整长窗不进入默认 CTest；跨 WRL／SIMPACK 的原始轨迹和全量统计留在版本控制之外，仓内只保留
少量摘要、对比图和短真实消费者。P179 的冻结控制算法、转矩指令调理和 100 Hz 事件已落位；
下一阶段以其 SIMPACK Realtime／冻结 WRL 工件完成 `30 s` 控制长窗。Radau3、SH17、编组和后续
MPC／学习控制均不在当前主线。
...
CVODE 时域积分和确定性八接口并行已形成完整车辆系统；随包 JSON 资产是运行时唯一输入。
...
- **P179 同人格控制起步段（0.5 s）**：接通 100 Hz 全状态控制、转矩指令调理与原子事件；该窗口
  尚未到达 `50 m` 导向区或 AAR5 激活区，受控 `30 s` 长窗留给 G77。
