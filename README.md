...
输入的 10 s 激活窗和 20 s 完整包络，以及 R300+AAR5 16 s 工况的线路几何与纵向站位传播；
四个轮对均已和 SIMPACK、WRL 作原生时序与里程序列核对。R300+AAR5 的横向车辆响应、IRW 与
控制系统尚未完成资格验证。实施顺序与完成条件见
...
`20 s` 完整包络的精度—性能基线。G62 又完成了 R300 线路、P040 AAR5 无损资产和 `16 s`
纵向站位传播资格；下一阶段复用同一工件裁决横移、摇头和轮轨力。历史报告中的
...
当前已完成四层真实计算：直线、水平、零超高且无轨道不平顺的 `10 ms` 首次闭环；同一车辆加入
冻结 AAR6 横／垂不平顺后的 `10 s` 激活窗；从 `t=0` 独立连续运行的 `20 s` 完整包络；以及
R300+AAR5 `16 s` 的线路与纵向站位资格。所有长窗都采用 60 km/h 初始速度和 `0.5 ms` 观察周期，
按原生时间序号和原生样本越站时刻比较，不作移时、插值、滤波、拟合、缩放或去均值。
