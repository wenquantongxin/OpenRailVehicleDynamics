...
计算性能按独立的
[双车型性能迁移路书](docs/performance/PERFORMANCE_MIGRATION_ROADMAP.md)推进。历史 GZ18 专项分支
曾使两条长窗的运行器分段总计快于实时，但该数字不是完整进程计时，也不代表现行 `main`；公共优化
将在当前 GZ18／IRW 架构上重新落位，按候选性质分层资格并最终以五条冻结长窗收口。通过后直接
替换旧实现，不保留兼容开关。
...
- `5/10/30 s` 级运行同时报告实际墙钟、动力学推进实时因子、端到端实时因子及分段耗时；不从起步
  ...
  牺牲已资格响应为代价。
- 除明确声明的逐位等价结构优化外，不保存数值金标、输出快照或文件哈希，也不把逐位一致作为一般
  数值资格要求。
