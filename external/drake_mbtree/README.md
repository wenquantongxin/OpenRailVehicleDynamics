...
源码分发须携带 vendored 源码对应的材料；二进制包由后续发布工作流按其实际包含的代码与库
重新收集许可证和 NOTICE。第一方代码自身的许可证尚未选定，见仓库根 `README.md`。
...
这是一道对 CMake 图的架构性尽力检查，不是最终证明：把检查延迟到目录末尾可以买到更晚的
观察时机，但不会把配置期表示变成最终链接产物，目录作用域 imported target 也可能从检查处
不可见。G31 另对已构建候选用 `file(GET_RUNTIME_DEPENDENCIES)` 检查按当前平台规则解析出的
动态运行期闭包；产物缺失或闭包存在未解析节点都直接拒绝。该检查只声称共享 Drake 不在这条
闭包中，不声称识别静态归档；普通静态/共享输入仍由本闸门早失败，静态对象身份由所有权与来源
审计证明。
