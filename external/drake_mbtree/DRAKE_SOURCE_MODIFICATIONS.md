...
比较过一次,结论只用于把位姿组合登记为后续性能资格工作的候选专项;那次比较的耗时不写进
源码,也不构成任何性能承诺。
...
处理由后续性能资格工作流在真实调用比例下判断,不由公共头的描述预先声明。
...
**缓存目录成为类型。** 十一个保留缓存是 `RigidMultibodyTreeCacheWorkspace` 的具名成员,
每个是 `VersionedCacheSlot<Value, Sources...>`,依赖集合写在类型参数里。工作区由求值上下文
以 `mutable` 成员私有承载——求值对模型而言是读,它不产生调用方没要过的东西,也不动状态;
这一点由成员是 `mutable` 表达,而不是在每个求值点写一次 `const_cast`。

**反向指针消失。** `tree_system_`、`MultibodyTreeSystem`、`GetParentTreeSystem`、
`MultibodyForces` 的 system 重载全部删除,`multibody_tree_system.h` 不再被包含。落位树的
编译前沿缺口因此归零。

**前向动力学加速度入口删除而非重绑。** 它不接受任何外力却返回加速度,等于替某一组特定外力
作答而不说是哪一组。绑成缓存会把这一点连同一个说不清来源的新鲜度戳一起保留。仅依赖它的
便利入口一并移除;接收已知 `vdot` 的入口不受影响,落位 ABA pass 留给 G36 组装。

**`BlockSystemJacobianCache` 整体退出。** 运行时契约第 5 条对它的处置是「不预分配缓存、
首版直接计算」,它当前无活消费者。账本由 `vendor` 改判 `discard`。

**零四元数拒绝落在完整 q 提交门。** 零四元数不是旋转,也无法被归一化成旋转——方向信息不是
不准,而是不存在。非零但非单位的四元数按原值接受并保存:在这里归一化会悄悄回答另一个问题,
而调用方永远不会知道读回来的不是写进去的。哪些段是四元数属于模型知识,所以门在树上而不在
状态存储里。

**上下文只能经模型感知工厂创建。** 裸构造函数收为私有,树为友元。别的路径造出来的上下文
持有存储层自己的默认值——零位置、无质量的刚体——而且它会照常求值,回来的是对一个没人建过的
模型做的算术。

### G27 复核收口

- 求值上下文只公开缓存工作区的 const 视图；可变工作区收为 tree 私有友元入口，调用方不能
  写任意缓存值后自行提交为“新鲜”。
- 删除返回完整 `MultibodyStateInstance&` 的参数入口。刚体惯量、固定坐标系位姿与执行器
  参数分别经具名 tree 入口写入；每个入口先核对 context 属于本 tree，再委托元素的类型化
  整记录 setter，不暴露可绕过 q/v 门的状态对象。
- 完整 q 写入先核对维数，再读取 quaternion 分段；短向量不会触发 Eigen 断言或越界。
  `SetDefaultState()` 同样汇入 tree 的 q/v 提交门，因此零默认 quaternion 会使工厂创建失败，
  而不会进入首次运动学求值。
- q/v 分段 setter 以明确的逐元素复制写入事务临时量。原先把一元素定长向量经动态
  `Eigen::Ref` 赋给动态 block 的写法会在优化构建中触发包加载越界诊断；控制面 setter
  不需要为这次短复制冒险走向量化路径。写入前显式核对起点与范围，空的越界 segment
  也不能以“未写值”为由推进版本。
...
`MultibodyTree::SetPositionsAndVelocities()` 是 ODE 试算向量进入 landed tree 的模型感知门。
它先核对状态归属、q/v 维数与每个 quaternion 的真实数值定义域，再让 G21 状态存储同时验证
两段有限性和两个版本后继；全部成功后才写 q/v 并各提交一次版本。该入口由已有的独立 q、v
