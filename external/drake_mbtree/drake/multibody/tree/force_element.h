...
class ForceElement : public MultibodyElement<T> {
 public:
  ...
  T CalcNonConservativePower(
      ...
      const internal::VelocityKinematicsCache<T>& vc) const {
    this->ValidateStateInstance(context.state());
    return DoCalcNonConservativePower(context, pc, vc);
  }
 ...
 protected:
  virtual T DoCalcPotentialEnergy(
      const orvd::rigid_multibody_tree::internal::
          RigidMultibodyTreeEvaluationContext& context,
      const internal::PositionKinematicsCache<T>& pc) const = 0;

  virtual T DoCalcConservativePower(
      const orvd::rigid_multibody_tree::internal::
          RigidMultibodyTreeEvaluationContext& context,
      const internal::PositionKinematicsCache<T>& pc,
      const internal::VelocityKinematicsCache<T>& vc) const = 0;

  virtual T DoCalcNonConservativePower(
      const orvd::rigid_multibody_tree::internal::
          RigidMultibodyTreeEvaluationContext& context,
      const internal::PositionKinematicsCache<T>& pc,
      const internal::VelocityKinematicsCache<T>& vc) const = 0;
  ...
  virtual void DoAssignForceElementParameterSlots(
  ...
  virtual void DoWriteDefaultForceElementParameters(
 ...
 private:
  ...
  void DoAssignParameterSlots(
      orvd::rigid_multibody_tree::internal::MultibodyParameterSlotAllocator*
          allocator) final {
    DoAssignForceElementParameterSlots(allocator);
  }
  ...
  void DoWriteDefaultParameters(
    ...
    DoWriteDefaultForceElementParameters(state);
  }
};
