...
class JointActuator final : public MultibodyElement<T> {
 public:
  ...
  /// @throws std::exception if this element has no parent MultibodyTree.
  ...
  /// @param[in,out] u
  ///   Actuation values for the entire multibody model to which `this` actuator
  ...
  /// @throws std::exception if this element has no parent MultibodyTree.
  /// @throws std::exception if `u.size()` differs from the parent
  ///   MultibodyTree's number of actuated degrees of freedom.
  ...
  /// Returns -1 if this %JointActuator has not been added to a MultibodyTree.
