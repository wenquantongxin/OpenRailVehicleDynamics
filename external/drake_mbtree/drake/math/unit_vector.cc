...
Vector3<T> NormalizeOrThrow(const Vector3<T>& v,
  ...
  if (!(isfinite(norm) && norm >= kMinMagnitude)) {
    throw std::logic_error(fmt::format(
        ...
        function_name, fmt_eigen(v), norm, kMinMagnitude));
  }
  return v / norm;
}
...
void ThrowIfNotUnitVector(const Vector3<T>& unit_vector,
  ...
  const double uvec_squared = unit_vector.squaredNorm();
...
template Vector3<double> NormalizeOrThrow<double>(const Vector3<double>&,
                                                  std::string_view);
template void ThrowIfNotUnitVector<double>(const Vector3<double>&,
