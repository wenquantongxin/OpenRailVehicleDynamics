...
//
// The document states its own schema version, the identifier the asset is known
// by, which of the two surfaces it describes, the coordinate convention its
// numbers are in, and the length unit they are expressed in. Naming the frame
// and the unit in the file is the point: a profile is a bare list of numbers,
// and a list of numbers that does not say what it means is one silent sign
// convention away from a wheel that climbs its own flange.
//
...
// unsupported schema versions, non-finite or underflowing floating-point
// tokens, and integer tokens that cannot be converted to binary64 without loss.
