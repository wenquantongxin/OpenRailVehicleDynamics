...
当前 `SystemContinuousStateAdvancer` 独占一个预先配置的试算上下文，`SystemRhsBridge` 只借用它；
...
边界只提交一次；失败后必须从仍有效的接受端点显式同步。同步入口逐类枚举当前准入的上下文局部
物理参数——关节阻尼与标称力——不建立参数袋，也不在 RHS 热路径扫描状态、名称或参数。
...
参数、窗口越线路域及成功端点投影失败继续终止公开事务，不能由诊断字符串或统一兜底降级。

事件仍待真实消费者出现时扩展，不以注册表或空回调预占接口。系统层当前不为无人消费的内部步
建立回调注册表；G55 的真实采样消费者已增加一次公开推进内的窄密集状态批，采样只读各成功内部步
