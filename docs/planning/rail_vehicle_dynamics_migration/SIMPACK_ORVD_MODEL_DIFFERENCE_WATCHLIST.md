...
- MD-002：GZ18 活动曲线的超高模式整数与实际滚转基准尚未建立可靠映射。**不阻塞随库 G51 示例**：
  示例资格域仍是直线、水平、零超高，`kind` 在那里不可观测。通用科研入口虽允许非零超高启动，
  但不因此获得参考工具对齐资格；应在首个正式承载非零超高或曲线的端到端场景前核实。
- MD-007：三维互穿纵向长度解析失败时，QCH 未公开处置语义；近期 WRL 使用解析弦长基线并计数。
  项目负责人已裁决恢复 WRL 兼容语义，G52b 后续修复已经落实。
- MD-008：Kalker 表外渐近公式有理论出处，但 SIMPACK 在 `a/b=0.1/10` 的精确切换语义尚未资格化；
  当前迁移继续复现 WRL 的硬切换，不以车辆响应边界规避，也不擅自平滑。
...
失败时保留基线、置回退标志，并累计尝试数与回退数。2026-08-02 的平顺 30 s 正式 RHS 工件记录
302/10,934,468 次回退／尝试，AAR5 30 s 工件记录 4,686/23,936,829 次。计数包含重复 RHS 和被拒步
试算，不能解释成唯一接受态样本，但证明该分支不是结构性不可达。

ORVD 提交 `8990dd4` 改为几何生产端和法向消费端双重强失败。接触尚未进入系统 RHS，因此当前不会终止
车辆仿真；G53 接线后，现有 CVODE 桥会把异常当作不可恢复失败，行为将与近期 WRL 正式路径不同。
...
覆盖、失败从同一次结果报告。不得为计数重新求值，不在热路径格式化日志或动态分配；通用 CVODE 可恢复
错误分类不随本条扩建，其他真实配置错误和内部不变量破坏仍按致命事务处理。
...
渐近式可追溯到 Kalker 1990 Appendix E/Table E3，表中注明渐近值来自 Kalker 1972a；因此它不是无来源
公式。WRL 与 ORVD 当前都选择 `kAsymptotic`。尚无证据证明 SIMPACK 的边界和拼接方式相同。有限表与渐近
式在下端的 C11/C22/C23 跳跃约为 -0.833%/-2.134%/-5.638%，上端约为
...
- WRL `scripts_cpp/rigid_wheelset/generated/gz18_contact_config.gen.h:141`：活动人格允许表外渐近式。
