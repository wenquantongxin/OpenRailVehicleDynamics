...
**几何工作区容量**：✅ `envelope_capacity_ = 4951`（由**未旋转**跨度 `0.099000000000000005 / dy_bin` 得出），
而名义姿态下需要 `nbins = 4979`，首次 `Solve` 有 **14 个向量**扩容（`zmax_bins, argmax_bins, y_env, z_env,
x_env, x_track_env, s_env, source_index_env, w_slope_env, wx_slope_env` 与 `env_coeffs, xtrack_coeffs,
pchip_h_scratch, pchip_delta_scratch`）。三点更正：增长**依赖符号**（左解算器只在 `phi=-atan(1/40)` 增长），
横移**无影响**，且这是**每工作区一次性**（实测约 102 µs，对 241.9 µs 的稳态 `Solve`），不是吞吐问题。

旋转不变容量：✅ 型面 y-z 包围盒对角线 `0.10313685179443649 m → 5158 bins`，比 4951 多 4.181%；
360° 侧滚扫描（1 mrad 步长）最坏 5085，**恒不越界**。但它**不覆盖偏航**：实测 `|psi| > 0.058562 rad` 即失效，
因为偏航把纵向坐标注入横向（`R0=0.42`，型面 `max|dz/dy|=2.6915`，都在二维包围盒之外）。
✅ 另有更省的路子：实测 `env_size = 1000 = s_samples`，4979 个 bin 只用了约 20%，占用 bin 数不可能超过采样数，
14 个向量里有 12 个可按 `s_samples` 定容。

**CodeX 复核（2026-08-05）**：上述分配次数只用于定位当前实现的分配源，不固化为产品金标。G52 的合同是
“模型／上下文构造期完成定容，促进路径求值不构造或扩容动态存储”；容量按 GZ18 类型化离散参数和型面包络
推导，并用已资格化姿态范围验证即可。不要为了 360° 姿态或算术极端再建一层防御框架；超出明确资格化范围的
科研调用由调用方负责。`ContactKernelOutput` 的四个单元素初始化列表、FASTSIM 切片和几何 scratch 是本轮应消除的
三处真实热路径分配。
...
1. ◻ GZ18 实际呈现给几何求解器的 `|psi|` 范围——这决定偏航盲的旋转不变容量是否安全（阈值 `0.058562 rad`）。
2. ◻ `a/b` 是否会离开 Kalker 表定义域 `[0.1,10]`。实测 GZ18 落在 `[1.115, 2.005]`，但 GZ18 开启了
   `allow_asymptotic_outside_table`，而该分支与表在边界不接（`a/b=0.1` 处 `c23` 跳 5.64%）。
3. ◻ 接触站位是否由投影获得。ORVD 的 `ProjectPointNearSeed` 只接受种子加窗口、拒绝越域窗口、
   且要求窗口内恰有一个内部极小；WRL 的 `ComputeSEff` 在 `rwc_core` 之外有三份逐字副本。
4. ◻ G53 的轨侧反力最终保留在规范点 `p_GP`，还是搬运到钢轨材料点——这是有了真实钢轨接线才能裁定的
   建模问题，不阻塞 G52a/G52b；G52 冻结同点约化的规范成对扳手和正确的空间扳手搬运原语。

**CodeX 收口判定（2026-08-05）**：SH17 已由 PREF-013 排除，不是未知项；G51 的单一轨型竖向偏移和车辆级
型面绑定也不需要返工。上述四项里，前两项须在 G52c 接入真实 GZ18 资产和人格时查清或如实收窄资格化范围；
后两项属于 G53 接线，不得用来拖延 G52a 开工。Claude 在任何子步遇到会改变这些架构边界的新事实，应停在该子步提交前
上呈，不得跨过 CodeX 阶段审查自行扩面。
