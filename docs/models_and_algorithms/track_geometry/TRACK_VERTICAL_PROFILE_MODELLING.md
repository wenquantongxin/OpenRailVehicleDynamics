...
本篇说明 ORVD 如何用水平投影站位上的恒坡段、抛物线竖曲线和圆弧竖曲线构造竖向剖面，如何用显式五次接缝连接相邻段，以及竖向剖面如何与平面曲率和超高共同形成三维中心线与轨型系。公式给出数学模型及其代码实现所采用的解析关系。

## 1. 范围与记号

轨道惯性系记为 `I`，其 `+x` 沿线路起点处的站位增加方向，`+y` 指向面向站位增加方向时的右侧，`+z` 向下。站位 $s$ 是中心线在水平面内的投影弧长，而不是三维中心线弧长。完整坐标约定见[坐标与记号约定](../CONVENTIONS_AND_NOTATION.md)，平面曲率、中心线积分和轨道坐标系的一般算法见[线路几何与轨道坐标系](TRACK_GEOMETRY_AND_FRAMES.md)。
...
| $\ell$ | 三维中心线弧长 |
| $\kappa_v$、$r_v$ | “水平投影站位—向上高程”平面内竖剖面的有符号曲率及其有符号半径 |
...
一条竖向剖面由首尾相接的有限段序列组成。每个原始段解析给出 $g$、$g'$、$g''$ 以及从该段起点累计的 $\int g\,ds$；整条剖面的 $I_g$ 由前面各片的积分与当前片的局部积分相加得到。ORVD 当前实现三种原始段：
...
因此高程使用接缝替换后的面积，而不是继续累计被覆盖的原始段。接缝坡度在每个窗口端点匹配所取用原始段的 $(g,g',g'')$。若该端点位于所取用原始段的内部，则接缝在该端点达到 $C^2$；与完整剖面的定义端点重合时，同一结论按剖面域内的单侧意义成立。相应的高程由 $z'=-g$ 得到，在该端点具有连续到三阶的导数，四阶导数通常可以跳变。若半窗恰好铺满相邻原始段的全长，该段被接缝完全覆盖而不再单独成片，窗口端点便与另一处未设接缝的原始段界重合；此时端点两侧分属接缝公式与再外一个原始段，只保证坡度值连续，$g'$ 可以跳变，高程只保证达到 $C^1$。左右两端对称存在这一情形。
...
因此，竖向原始段界、接缝窗口端点与平面曲率断点共同进入中心线节点划分。被接缝窗口覆盖的原始边界仍保留在线路拓扑中，但竖向公式的实际切换发生在接缝窗口两端。
...
竖向位置始终由 `TrackVerticalProfile` 的解析 $I_g$ 形成。水平位置只依赖 $\kappa$：常曲率区间使用圆弧弦的闭式，变曲率区间使用分面板的固定阶 Gauss–Legendre 求积。中心线节点表建立在平面曲率断点、竖向原始段界与接缝窗口端点的有序并集上；每个断点区间再等分为若干长度不超过声明节点间距的面板，并在定义区间末端追加终点节点。因此每个节点区间既不跨越水平积分公式的切换，也不跨越 $\mathbf C''$ 的分片位置。超高不改变中心线，所以超高段界不单独产生中心线积分节点。
...
令 $n=\sqrt{1+g^2}$、$\mathbf t=\mathbf C'$，无侧滚切向系的三轴为
...
轨型系的站位旋转率由与位姿相同的 $\kappa$、$g$、$g'$、$u$ 和 $u'$ 构造。无侧滚轴的导数为
...
当前实现采用中心线基准：超高只旋转轨型系，$\mathbf C(s)$ 不随 $u$ 平移。超高参考基长 $b$ 是抽象参考基线的长度，不等同于名义轨距或轮轨型面定位距离。
...
平面曲率、竖向剖面和超高在组合时共享有限定义区间 $[s_{\min},s_{\max}]$。对区间外的有限站位，令 $s_\ast\in\{s_{\min},s_{\max}\}$ 为最近的定义边界，组合后的线路几何采用三维切线延长：
...
域外保持边界的 $\psi$、$g$、$u$ 和 $\phi$，并令 $\kappa=g'=u'=0$。因此中心线和轨型系姿态在定义边界连续，但中心线二阶导数与轨型系旋转率可以发生跳变。这是组合线路的明确延拓，不是由区间内最后一个 PL2 或 CIR 唯一推出的外推。

### 8.2 理想竖向线形与不平顺

理想竖向剖面决定中心线高程、坡度和轨型系。竖向不平顺是以轨型系表达并叠加在理想线路上的局部位移场，不改写 $g(s)$、$I_g(s)$ 或理想中心线。车辆沿线路采样不平顺时产生的位移变化率属于轮轨相对运动，不是新的恒坡段、PL2 或 CIR。
...
- 无接缝边界只保证坡度值连续，$g'$ 与更高导数可以跳变；五次接缝在窗口端点匹配所取用原始段的 $(g,g',g'')$，$g'''$ 通常仍可跳变，而装配后的剖面在该端点是否达到 $C^2$ 由第 5.3 节的条件决定。
...
| 三维中心线、切线延长与轨型系运动学 | `TrackGeometry`、`EvaluateTrackFrame` | [`track_geometry.h`](../../../libs/track_geometry/include/orvd/track_geometry/track_geometry.h)、[`track_geometry.cc`](../../../libs/track_geometry/src/track_geometry.cc) |
