...
Only the notation this document adds is listed below; for $s$, $\psi$, $\kappa$, $g$, $u$, $b$, $\phi$, $\mathbf C(s)$, $R_{IT}$, $R_{I0}$, $\mathbf x_0,\mathbf y_0,\mathbf z_0$ and $\boldsymbol\omega_{IT}$ see Sections 2 and 3 of [Conventions and notation](../CONVENTIONS_AND_NOTATION.en.md).

| Symbol | Meaning | Code identifier |
|---|---|---|
| $[s_{\min},s_{\max}]$ | Definition interval of the line geometry; its endpoints are the start and end stations of the curvature profile, defined in Section 3.5 | `start_track_station_meters`, `end_track_station_meters` |
...
| $x=\hat s/L_i$ | Normalized coordinate of a Hermite blend segment | `kHermiteCubicBlend` |
...
| $\xi$, $d_r$, $H_r(\xi)$ | Normalized window coordinate, the six boundary data, the quintic Hermite basis functions | `kQuinticBasis` |
...
| $h$ | Half turn angle over $L$ on a constant-curvature node interval | `half_turn` |
| $\mathbf t$, $n$ | The unnormalized three-dimensional tangent $\mathbf C'(s)$ and its norm $\sqrt{1+g^2}$ | `tangent`, `slope_norm` |
...
A scalar profile is a sequence of `TrackScalarSegment` laid end to end from `start_track_station_meters`; each segment length `length_meters` must be finite and strictly positive, and the segment end must be a finite station strictly after its start. A segment has only two shapes. `TrackScalarSegmentShape::kConstant` holds `start_value` over the whole segment; `TrackScalarSegmentShape::kHermiteCubicBlend` blends the value from `start_value` to `end_value` through the normalized coordinate $x$:
...
Differentiating with respect to $s$ and expressing the result in $x$ exposes the end behaviour of this blend:
...
The normalized coefficients $a_k=\sum_r d_r\,K_{rk}$ ($K$ being that table) are then rescaled by power back into the physical local coordinate whose origin is the window start:
...
The definition of the quintic bridge requires a finite, strictly positive width $w$, a window centered on an internal boundary with adjacent segments on both sides, at most one window per boundary and a half-width $w/2$ no greater than either adjacent segment length. Windows must not overlap, although touching endpoints are allowed. These conditions give every set of six boundary data a unique source and make the resulting piece partition single-valued over the whole station domain.
...
Definition of the signed superelevation: take two abstract reference points on the track-frame transverse axis $\mathbf y_T=R_{IT}\mathbf e_2$ at $b/2$ on either side of the centerline, $\mathbf p_{\text{right}}=+\tfrac{b}{2}\mathbf y_T$ and $\mathbf p_{\text{left}}=-\tfrac{b}{2}\mathbf y_T$; then $u$ is their signed separation along the roll-free vertical axis $\mathbf z_0$:
...
The line geometry is defined on the finite station interval $[s_{\min},s_{\max}]$, whose endpoints are the start and end stations of the curvature profile (all three profiles have been canonicalized to a common end at construction, Section 4.2). `ClassifyTrackStation` returns `TrackStationRegion::kBeforeDefinedInterval` for $s<s_{\min}$, `kAfterDefinedInterval` for $s>s_{\max}$ and `kWithinDefinedInterval` otherwise; both endpoints belong to the interval. For any finite station outside it, `TrackGeometry` continues along the three-dimensional tangent of the nearest endpoint $s_\ast\in\{s_{\min},s_{\max}\}$ as a straight line ($s_\ast$ is an endpoint of the definition interval and is unrelated to the seam boundary $s_b$ of Section 3.2):
...
In the implementation `CenterlinePositionUnchecked` combines the endpoint position and endpoint derivative for an out-of-domain station. Heading, superelevation, grade and roll take their endpoint values, while curvature, the centerline second derivative and the rates of grade and superelevation become zero. This tangent continuation belongs to the geometric definition of `TrackGeometry`; the underlying scalar profiles remain defined only on their finite intervals. It is an explicit continuation choice, not an extrapolation of the final analytic profile piece.

`support_start_track_station_meters()` gives the start of the profile support: it is the start station of the first piece whose polynomial coefficients are not all zero, and it has no value when the profile is identically zero. Because it examines analytic pieces rather than samples, it returns the window start when the first non-zero piece is a seam, not the original segment boundary. The vertical profile follows the same idea: a constant-grade original segment is classified by its start grade and a seam piece by whether all its coefficients vanish.
...
Here $\tau$ is `kObjectiveGradientRelativeTolerance`, whose value is `1.0e-10`. The residual bound scales with distance because $f'$ has units of length; the numeric constant `1.0` in the SI-coordinate implementation represents $\ell_0=1\,\mathrm m$ and provides a dimensionally consistent absolute floor at zero distance. Requiring $f''>0$ excludes the case where the distance stationary point is a maximum, for instance when the point lies beyond the curvature center of a circular curve. The model deliberately describes only a local branch: the caller supplies a station seed $s_{\text{seed}}$ that identifies the current branch, and this layer performs no whole-line closest-point search, scans no nodes and never reselects a distant root; the reasons and the algorithm are in Section 4.4.

## 4. Algorithm

### 4.1 Constructing and evaluating a scalar profile

Construction first accumulates station segment by segment, forms each original local polynomial as in Section 3.1 and forms seam intervals from internal boundaries and window widths. Before the piece sequence is cut, the geometric prerequisites of Section 3.2 and continuity of the declared values at every boundary not covered by a window must all hold. The windows are then sorted by start station, and a station cursor cuts out the part of an original segment before a window, the window itself and the part after it. A segment cut by a window retains the original segment start as its polynomial origin, avoiding extra rounding from re-expansion about the cut; a seam piece uses the window start. Finally, construction computes the accumulated integral $I_i$ at every piece start, records the breakpoint table (all piece starts plus the profile end) and finds the support start.
...
Each node stores its own station $s_n$, the heading $\psi_n$ (the exact integral `curvature_.IntegralFromStart`) and the centerline position. The horizontal displacement from a node $s_n$ to any station $s$ inside its interval is given by `HorizontalDisplacementFromNode`; in the two displacement formulas below $L=s-s_n$ is that station difference, not the segment length $L_i$ of the notation table. It is one whole panel while the node table is built and the part from the node to the evaluation station at a non-node evaluation, so $0\le L$ and $L$ never exceeds one panel length, the displacement being zero at $L=0$. Where the curvature is constant it uses the half-angle form of the circular chord:
...
A node's $z$ component is taken directly as `-grade_.IntegralFromStart` and takes no part in the running sum. The position at a non-node station is the position of the node found by the binary search `NodeIndexAtOrBefore` plus one call of `HorizontalDisplacementFromNode` from that node to the target station, so at most the part of one panel from that node to the target is integrated; a station landing on the last node belongs to the interval that ends there.

### 4.3 Track-frame kinematics

`EvaluateTrackFrame` produces the pose and the station derivatives in one pass. It first takes $\psi$, $\kappa$, $g$, $g'$, $u$ and $u'$ (zero or endpoint values outside the interval, per Section 3.5), then forms the three axes and the rotation as in Section 3.4. The station derivatives of the axes are
...
The algorithm defines no finite window around the seed and does not require the node grid to bracket the root; otherwise a window wall or node placement could change whether the same continuous branch is recognized. The corresponding locality assumption is strong: the current state must lie near a regular root with $f''>0$, and the seed must be in a Newton attraction region reachable in at most two corrections. The size of that region depends on curvature, grade and point-to-centerline distance; the code supplies no uniform radius.
...
The non-smooth points are the following. At the two ends of a Hermite blend, value and first derivative are continuous while the second derivative generally jumps; a degenerate $\Delta v=0$ segment or an adjacent segment whose end second derivative happens to match is an exception. At a boundary without a seam, value is continuous, the first derivative is continuous and zero, and the second derivative may jump; a junction of equal constants is entirely smooth. When a seam-window endpoint lies inside the raw segment that supplies its boundary data, value, first derivative and second derivative are continuous there while the third may jump. The same conclusion holds in the one-sided sense within the profile when the endpoint coincides with a definition endpoint of the complete profile. If a half-window exactly covers that raw segment and its endpoint also lies on a further raw boundary without a seam, the assembled profile is generally only $C^1$ there and its second derivative may jump. At a definition boundary, $\mathbf C$, $\mathbf C'$ and $R_{IT}$ are continuous, but $\kappa$, $g'$ and $u'$ become zero outside, so $\mathbf C''$ and $\boldsymbol\omega_{IT}$ may jump. Piece lookup selects the right-hand piece at an interior boundary and the last piece at the end, so derivative queries return one-sided values there. The derivative of $\phi=\arcsin(u/b)$ is unbounded as $\lvert u\rvert\to b$, hence the model is restricted to $\lvert u\rvert<b$.
...
| Quintic $(v,v',v'')$ seam | `internal::BuildQuinticHermiteCoefficients` | [`track_profile_quintic.cc`](../../../libs/track_geometry/src/track_profile_quintic.cc) |
...
| Track-frame attitude and station rotation rate | `EvaluateTrackFrame` | [`track_geometry.cc`](../../../libs/track_geometry/src/track_geometry.cc) |
| Three-dimensional tangent continuation outside the finite domain | `TrackGeometry::CenterlinePositionInInertialMeters` and the `CenterlinePositionUnchecked` it calls | [`track_geometry.cc`](../../../libs/track_geometry/src/track_geometry.cc) |
...
- Planar curvature and superelevation consist only of constant pieces, Hermite cubic blends and optional quintic seams. A seam matches $(v,v',v'')$ of the source raw segment at a window endpoint; if a half-window exactly covers that raw segment and ends at a further raw boundary, the assembled profile is generally only $C^1$ there. For a curvature transition from $\kappa_0$ to $\kappa_1$, the Hermite law $\kappa(s)=\kappa_0+(\kappa_1-\kappa_0)(3x^2-2x^3)$ is exactly the curvature law of a Bloss transition curve, not a clothoid. The current model has no linear-curvature clothoid, sinusoidal transition or arbitrary sampled alignment; those shapes cannot be represented losslessly by the existing segment types.
...
- Outside the finite definition interval, the model uses the endpoint three-dimensional tangent. This is a chosen geometric continuation, not a unique consequence of the curvature, grade or superelevation formulas inside the interval.
...
- Smoothness follows both the piece type and the raw topology at a window endpoint. Hermite blend endpoints are generally $C^1$. A quintic seam is $C^2$ with its source raw segment at an ordinary window endpoint, but may be only $C^1$ at a further raw boundary when its half-window consumes that segment. Tangent continuation preserves $\mathbf C$ and $\mathbf C'$ at a definition boundary, while $\mathbf C''$ and the track-frame rotation rate may jump.
