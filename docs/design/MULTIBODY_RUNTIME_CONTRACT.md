...
固定坐标系位姿这一入口代表同一上下文中的权威参数，而不只代表位置缓存的一个实现细节。
凡是把 body 原点的速度或加速度移到 fixed-frame 原点的计算，也必须经
`Frame::CalcPoseInBodyFrame(state)` 读取同一值；不能另读构造期 `X_PF_`。否则在同一个
上下文改写 frame pose 后，位姿、Jacobian 与速度/加速度会对 frame 所在位置给出互相矛盾的
答案。
