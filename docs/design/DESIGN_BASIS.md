...
- OpenMP：该早期分歧已由 G59 收口。现行实现不采用轮位→worker 映射，而是每个冻结接口永久
  独占运行时工作区；1/2/4/8 线程固定槽结果与完整 RHS 逐位一致，标准 OpenMP 进程环境控制资源，
  产品内部封顶八线程。权威合同见 ADR-0006 与现行迁移路书，本设计底稿不再承载候选状态。
